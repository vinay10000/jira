...
const app = new Hono()
    .get("/", sessionMiddleware, zValidator("query", z.object({ workspaceId: z.string() })), async (c) => {
        ...
        const populatedMembers = await Promise.all(members.documents.map(async (member) => {
            ...
            return { ...member, name: user, email: user.email };
        })
        );
    ...
    .delete("/:memberId", sessionMiddleware, async (c) => {
        ...
        const allMembers = await databases.listDocuments(DATABASE_ID, MEMBERS_ID, [Query.equal("workspaceId", memberToDelete.workspaceId)]);
    ...
    .patch("/:memberId", sessionMiddleware, zValidator("json", z.object({ role: z.nativeEnum(MemberRole) })), async (c) => {
        ...
        const allMembers = await databases.listDocuments(DATABASE_ID, MEMBERS_ID, [Query.equal("workspaceId", memberToUpdate.workspaceId)]);
